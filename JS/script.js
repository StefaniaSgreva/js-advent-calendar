...
//Elenco con gli indici delle finestrelle aperte
let openedIndexes = [];
...
------------------------------------*/
//! Controlliamo subito se c'erano degli indici salvati delle finestrelle aperte! 
...
for(let i = 0; i < source.length; i ++){
    ...
    //2. agganciamo al calendario in pagina
...
//Funzione per rempire la modale
...
function addToOpenedIndexes(i){
    //Se dentro OpenedIndexes NON c'è già la i
    if(!openedIndexes.includes(i)){
    //la aggiungiamo alla lista
        openedIndexes.push(i);
    //la aggiungiamo al localStorage per non perdere la memoria nel browser delle finestrelle già aperte
    //localstorage accetta solo stringhe perciò uso la funzione JSON.stringify per convertire in stringa
